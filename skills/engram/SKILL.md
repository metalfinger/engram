...
- **Organizing projects.** Group them with `kb_tag_project(project, tags=["client-work"])` — tags are the folders here, so a project can sit in several groups and nothing has to move on disk. `status: archived` tucks a finished project out of the way (kept, not deleted); the web home and sidebar group by tag and collapse archived ones.
